...
/**
 * Resolves up to 75 card identifiers in one Scryfall request.
 * @param {Array<object>} identifiers Scryfall collection identifiers
 * @returns {Promise<{data: Array<object>, not_found: Array<object>}>}
 */
export async function fetchCardCollection(identifiers) {
  if (!Array.isArray(identifiers) || identifiers.length < 1 || identifiers.length > 75) {
    throw new Error("Scryfall collection requests require between 1 and 75 identifiers.");
  }

  const response = await fetch("https://api.scryfall.com/cards/collection", {
    method: "POST",
    headers: SCRYFALL_JSON_HEADERS,
    body: JSON.stringify({ identifiers })
  });
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.details || data.error || `Scryfall collection lookup failed with status ${response.status}`);
  }
  return data;
}
